...
path_json = r'D:\HKUST\00_Work\04_Facade\03_Database\json'  # 这里是指.json文件所在文件夹的路径
                             # 批量转换，修改此路径
                             # 此路径为，json文件所在路径
    ...
    for eachfile in os.listdir(path_json_to_data):
        ...
        if os.path.isdir(path1):                           #判断path1路径是否存在
            if os.path.exists(path1 + '/label.png'):       #判断path1路径下label.png是否存在
                ...
                path2 = os.path.join(path_save_png, (eachfile.split('_')[0] + '_' + eachfile.split('_')[1] + '_' + eachfile.split('_')[2] + '.png'))
  # 将png复制到path2路径下的文件夹中去
